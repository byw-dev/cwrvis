...
- 展示变量：15 个（`dxy` 为网格面积辅助变量，不参与展示和预生成）
- 年颗粒度：2000–2015，16 帧；月颗粒度：2000-01 至 2025-12，312 帧
- 总文件数：31 个（1 meta + 15 var × 2 颗粒度），总体积约 30MB（gzip 约 8MB）

**文件结构**：
```
/static/grid/
├── meta.json              # 网格坐标、时间轴、var 元数据
├── year/{var}.json        # 年颗粒度数据
└── month/{var}.json       # 月颗粒度数据
```
