...
> Task F-21（Enhancement）：在格点色彩图层可见时，区域边界线条因透明度过低而难以辨认，改善视觉对比度。

**改善方案**：双层 LineLayer 实现光晕效果——底层为较宽的深色/黑色半透明线（描边），顶层保留原有青色细线。同时适当提升悬停和选中状态的 fill-opacity。

| 状态 | 底层描边（新增） | 顶层线（调整） | fill-opacity |
|------|-----------------|----------------|--------------|
| 地市边界（常态） | `rgba(0,0,0,0.3)` 3px | `rgba(88,224,255,0.5)` 1.5px | — |
| 悬停 | — | — | 0.20（原 0.12）|
| 选中 | `rgba(0,0,0,0.4)` 4px | `rgba(88,224,255,1.0)` 2px | 0.35（原 0.25）|
