...
```
[◆ 云水资源显示分析平台  CWR·DATA PLATFORM]  [01总览][02空间分布][03区域评估][04时序分析][05站点观测][06模式诊断][07报告制作]  [? ⚙ 研究员]
```
...
五边形 SVG 图标 + "云水资源显示分析平台" + 副标题 "CWR · DATA PLATFORM v0.x"（单色，`--accent` 色图标）。
...
在空间分布 SubToolbar 的基础上，在聚合模式选择器右侧追加**区域选择器**：
...
56px 宽纵向图标条，仅在空间分布（`grid`）和区域评估（`region`）模块中显示。
...
| Tab | granularity | 帧数 | 数据来源 | 展示方式 |
|-----|------------|------|---------|---------|
| 逐月 | `month` | 312 | statsCache 或后端请求 | 折线图 |
| 逐年 | `year` | 26 | statsCache 或后端请求 | 折线图 |
| 月平均 | `mean_month` | 12 | statsCache 或后端请求 | 折线图 |
| 季平均 | `mean_season` | 4 | statsCache 或后端请求 | 折线图 |
...
- 区域选择器：与区域评估模块相同的下拉组件（复用）
- 年份下拉：`全部` / `2000` / `2001` / … / `2025`
- 点击下载 → 显示加载遮罩（伪进度条，F-29）→ `GET /api/v1/report/download?region_id=...&year=...`
- 后端成功响应（含 8~12s 模拟延迟，B-08）→ 进度条跳 100% → 浏览器触发下载 → 关闭遮罩
- 后端 404/400 → 立即关闭遮罩 → 页面内错误提示（无延迟）

**伪进度条遮罩**（F-29）：
- 覆盖模块内容区域（非全屏）
- 文字：`正在生成报告，请稍候...`
- 进度条：线性推进，约 9s 到 90%，停在 90% 等待响应；收到响应后跳至 100%，300ms 后关闭
- 遮罩显示期间下载按钮禁用
...
仅在空间分布（`grid`）和区域评估（`region`）模块显示。空间分布与区域评估**共享同一时间轴状态**（Pinia `stores/time.ts`），切换 Tab 时时间选择保留。
...
| 量程模式 | `'auto' \| 'preset'` | `cwrvis:scale_mode` | 全局 | `'auto'` |
...
interface SettingsState {
  basemap:    BasemapId;
  scaleMode:  'auto' | 'preset';              // 量程模式：自动 or 预设（来自 colorbars.ts）
...
**优先级（从高到低）**：
1. **用户手动输入**：图例阈值输入框填写的 min/max，优先级最高，跨帧保留。
2. **预设量程**（`scaleMode === 'preset'`）：从 `config/colorbars.ts` 静态查表取值（见下）。
3. **自动量程**：每帧从当帧数据独立计算 min/max。

**预设量程查表规则**：
- 数据来源：`data/colorbars/` 下 4 个 CSV 的 `colorbar_min`/`colorbar_max` 列，经 `make colorbars` 生成 `frontend/src/config/colorbars.ts`。
- 查表键：`{varName}_{gran}_{unit}`，其中 gran = `year`（`yearly`/`avg_yearly` 模式）或 `month`（`monthly`/`avg_monthly` 模式），unit = `mm`（kg 变量且 isKgToMm 开启）或 `kg`（其余情况）。
- **例外**：`avg_season`（季平均）模式始终走自动量程，忽略 `scaleMode` 设置。

**量程模式切换**：`scaleMode` 变化时，清空 imageCache 并重新渲染（与 colormap 切换行为一致）。

**量程清除时机**：切换 var 或聚合模式时用户阈值自动清空；切换单位（kg↔mm）时同样立即清空。

**超出量程着色**：低于 vmin 按 vmin 的颜色渲染，高于 vmax 按 vmax 的颜色渲染（clamp），而非透明。只有真正的缺测格点（null）才渲染为透明。

**相关文件**：`frontend/src/workers/gridRenderer.worker.ts`、`frontend/src/composables/useGridLayer.ts`、`frontend/src/components/panels/Legend.vue`、`frontend/src/config/colorbars.ts`（生成文件）
