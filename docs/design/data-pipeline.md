...
| **路径 B**：预计算 CSV → SQLite | `scripts/csv_to_sqlite.py` | 数据同事提供了统计口径一致的预计算 CSV | ⚠️ 待开发 |
...
| `make data-sqlite-csv` | 路径 B：运行 `csv_to_sqlite.py`，从预计算 CSV 导入（需要 `data/csv/` 就绪，⚠️ 脚本待开发） |
